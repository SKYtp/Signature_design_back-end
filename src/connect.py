...
def genImage(name, style, nothing_omega_loop):
    ...
    if consonant[0] in f_pos:
        c = consonant[0]
        pic = rgb2gray(generate(0,c,model_path,torch.tensor([style])))
        while final_value_contour_head(pic)[0]:
            pic = rgb2gray(generate(0,c,model_path,torch.tensor([style])))
            point = np.column_stack(np.where(pic <= 10))
            print(len(group_cal(point)))
        head = final_value_contour_head(pic)
        image.append(pic)
        if consonant[1] in s_pos:
            c = consonant[1]
            pic = rgb2gray(generate(1,c,model_path))
            while is_broken_line(pic,c) or compare_text_height(image[0],pic)[0]:
                pic = rgb2gray(generate(1,c,model_path))
            print(compare_text_height(image[0],pic)[1])
            ...
            if nothing_omega_loop == 0:
                ...
                for i in range(len(consonant)-3):    
                    if nothing_omega_loop == 1:
                        if consonant[1] in s_pos_top:
                            c = 'โอเมก้าหงาย'
                            pic = rgb2gray(generate(3,c,model_path))
                            while is_broken_line(pic,c) or compare_text_height(image[0],pic)[0]:
                                pic = rgb2gray(generate(3,c,model_path))
                            omega = 0
                            image.append(pic)
                ...
                if consonant[-1] in l_pos:
                    c = consonant[-1]
                    pic = rgb2gray(generate(2,c,model_path))
                    while is_broken_line(pic,c) or compare_text_height(image[0],pic)[0]:
                        pic = rgb2gray(generate(2,c,model_path))
...
def v_concat(name,style,nothing_omega_loop,tilt,dot,line):
    ...
    if dot:
        if line:
            ...
            h , w = image_concat1.shape
            point = [(w+3, 99)]
            image_concat1 = cv2.copyMakeBorder(image_concat1, 0, 0, 0, 10, cv2.BORDER_CONSTANT, value=(255, 255, 255))
    ...
    if tilt:
        image_concat1 = cv2.copyMakeBorder(image_concat1, 0, 20, 0, 10, cv2.BORDER_CONSTANT, value=(255, 255, 255))
...
if __name__ == "__main__":
    ...
    image_concat = v_concat(name,style,nothing_omega_loop,tilt,dot,line) 
